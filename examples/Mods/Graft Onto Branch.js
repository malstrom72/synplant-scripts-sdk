// Vendored from snippets/grafting.js. Keep this Mod self-contained for normal script distribution.
addModPatcher('Graft Onto Branch', 'Synplant2_main', function(cushyName, cushyContents) {
    ...
    function clonePatch(patch) {
        return unmarshal('patch', marshal('patch', patch, false));
    }
    ...
    mods.grafting = {
        copyBranch: {
            ...
            execute: function copyBranchExecute() {
                ...
                var spawnedPatch = copyBranchPatch(getElement('patch'), fromBranch);
        ...
        graftOntoBranch: {
            ...
            execute: function graftOntoBranchExecute() {
                ...
                var targetPatch = clonePatch(getElement('patch'));
                var sourcePatch = unmarshal('patch', sourceClip);
                var result = graftPatchOntoBranch(targetPatch, sourcePatch, ontoBranch, false);
                if (!result.ok) {
                    if (result.reason === 'target-retune-required') {
                        if (display(result.message, 'warning', 'ok cancel') !== 'ok') {
                            return;
                        }
                        targetPatch = clonePatch(getElement('patch'));
                        sourcePatch = unmarshal('patch', sourceClip);
                        result = graftPatchOntoBranch(targetPatch, sourcePatch, ontoBranch, true);
                    } else {
                        display(result.message, 'error');
                        return;
                    }
                }
                if (!result.ok) {
